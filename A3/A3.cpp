...
void A3::renderNode(const SceneNode * root, mat4 view, mat4 model) {
	...
	for (const SceneNode * node : root->children) {
		renderNode(node, view, tmp_model);
	}
}
...
void A3::resetJoints(SceneNode * node) {
	undo = stack<std::vector<JointState>>();
	redo = stack<std::vector<JointState>>();
...
void A3::undoOperation() {
	...
	vector<JointState> information = undo.top();
	saveState();
	redo.push(oldInfo);
	undo.pop();
...
void A3::redoOperation() {
	...
	vector<JointState> information = redo.top();
	saveState();
	undo.push(oldInfo);
	redo.pop();
...
void A3::applyPositionChange(double xPos, double yPos) {
	...
	if (xy_sqr > 1) { 
		trackball_pos /= sqrt(xy_sqr);
	} else {
		trackball_pos.z = sqrt(1 - xy_sqr);
	}
